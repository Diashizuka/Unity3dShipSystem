...
namespace RVO
{
    ...
    internal class Agent
    {
        ...
        internal float accelerated_ = 0.0f;
        internal float angularSpeed_ = 0.0f;
        internal float minSpeedToTurn_ = 0.0f;
        ...
        internal void computeNewVelocity()
        {
            ...
            if (accelerated_ > 0.0f && id_ == 2)
                Debug.Log("OldVelocity:" + newVelocity_);
            if (accelerated_ > 0.0f && id_ == 2)
                Debug.Log("NowVelocity:" + velocity_);
            if (accelerated_ > 0.0f)
                acceleratedProgram(velocity_, ref newVelocity_);
            if (accelerated_ > 0.0f && id_ == 2)
               Debug.Log("NewVelocity:" + newVelocity_);
        }
        ...
        private bool linearProgram1(IList<Line> lines, int lineNo, float radius, Vector2 optVelocity, bool directionOpt, ref Vector2 result)
        {
            ...
            /* float accRange = 5.0f;
            float accRangeSq = accRange * accRange;
            Vector2 acceleration = result - optVelocity;
            if(RVOMath.absSq (result - optVelocity) > accRangeSq)
            {
                result = optVelocity + RVOMath.normalize(acceleration) * accRange;
            } */
